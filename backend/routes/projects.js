...
const db      = require('../db');
const { logProjectEvent } = require('../history');
...
async function refreshStatus(projectId) {
  ...
  if (p && !p.status_forced) {
    ...
    if (newStatus !== p.status) {
      ...
      await logProjectEvent(projectId, 'status_changed',
        `Statut passé à "${labels[newStatus] || newStatus}" (automatique)`,
        p.status, newStatus);
    }
  }
}
...
router.post('/', async (req, res) => {
  try {
    ...
    await logProjectEvent(r.insertId, 'created', `Projet ${code} créé : "${name}"`);
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (status && status !== ex.status) {
      finalStatus  = status;
      statusForced = 1;
      await logProjectEvent(req.params.id, 'status_changed',
        `Statut forcé à "${statusLabels[status] || status}"`, ex.status, status);
    } else if (!ex.status_forced) {
      finalStatus = await computeAutoStatus(req.params.id);
    }
    if (name !== ex.name) {
      await logProjectEvent(req.params.id, 'name_changed',
        `Nom modifié : "${ex.name}" → "${name}"`, ex.name, name);
    }
    await db.query(
...
router.patch('/:id/status', async (req, res) => {
  try {
    ...
    if (force && status) {
      ...
      await logProjectEvent(req.params.id, 'status_changed',
        `Statut forcé à "${statusLabels[status] || status}"`, cur.status, status);
    ...
    if (s !== cur.status) {
      await logProjectEvent(req.params.id, 'status_changed',
        `Statut recalculé à "${statusLabels[s] || s}"`, cur.status, s);
    }
...
router.post('/:id/items', async (req, res) => {
  try {
    ...
    await logProjectEvent(req.params.id, 'item_added', `Pièce ajoutée : "${name}"`);
...
router.put('/:id/items/:itemId', async (req, res) => {
  try {
    ...
    if (old && old.name !== name) {
      await logProjectEvent(req.params.id, 'item_modified', `Pièce renommée : "${old.name}" → "${name}"`);
    }
...
router.delete('/:id/items/:itemId', async (req, res) => {
  try {
    ...
    if (item) await logProjectEvent(req.params.id, 'item_removed', `Pièce supprimée : "${item.name}"`);
...
router.post('/:id/items/:itemId/prints', async (req, res) => {
  try {
    ...
    if (pr && it) await logProjectEvent(req.params.id, 'print_linked',
      `Impression "${pr.name}" rattachée à la pièce "${it.name}"`);
    await refreshStatus(req.params.id);
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.delete('/:id/items/:itemId/prints/:printId', async (req, res) => {
  try {
    ...
    if (pr) await logProjectEvent(req.params.id, 'print_unlinked',
      `Impression "${pr.name}" retirée du projet`);
    await refreshStatus(req.params.id);
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.post('/:id/prints', async (req, res) => {
  try {
    ...
    if (pr) await logProjectEvent(req.params.id, 'print_linked',
      `Impression "${pr.name}" rattachée directement au projet`);
    await refreshStatus(req.params.id);
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
