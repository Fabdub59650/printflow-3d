...
function parseMultipart(req) {
  return new Promise((resolve, reject) => {
    ...
    if (!bMatch) return reject(new Error('boundary manquant'));
    ...
    req.on('end', () => {
      try {
        ...
        const fields = {};
        let fileBuffer = null, fileName = '', fileExt = '';
        let pos = buf.indexOf('--' + boundary) + boundary.length + 4;
        while (pos < buf.length) {
          ...
          if (fileM && nameM) {
            fileName = fileM[1]; fileExt = path.extname(fileName).slice(1).toLowerCase();
            fileBuffer = Buffer.from(body);
          } else if (nameM) { fields[nameM[1]] = body.toString().trim(); }
          if (next === -1) break;
...
router.post('/files', async (req, res) => {
  try {
    const libPath = await getLibraryPath();
    const { fields, fileBuffer, fileName, fileExt } = await parseMultipart(req);
    if (!fileBuffer||!fileName) return res.status(400).json({ error: 'Aucun fichier reçu' });
