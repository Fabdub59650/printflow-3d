...
// GET /api/stats/alerts — bobines sous seuil d'alerte
router.get('/alerts', async (req, res) => {
  try {
    const [[setting]] = await db.query(
      "SELECT value FROM settings WHERE key_name='stock_alert_threshold'"
    );
    const threshold = parseInt(setting?.value) || 20;
    const [rows] = await db.query(`
      SELECT id, name, material, color_hex, color_name, brand,
             weight_remaining, weight_total,
             ROUND((weight_remaining/weight_total)*100) AS pct_remaining,
             diameter,
             ? AS threshold
      FROM filaments
      WHERE archived=0
      ...
      ORDER BY (weight_remaining/weight_total) ASC
    `, [threshold, threshold]);
    res.json(rows);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
