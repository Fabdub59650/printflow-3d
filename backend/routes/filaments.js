...
router.patch('/:id/weigh', async (req, res) => {
  try {
    const { weight_remaining } = req.body;
    ...
    await db.query(
      ...
      [Math.max(0, parseFloat(weight_remaining)), req.params.id]
    );
    const [[f]] = await db.query('SELECT name FROM filaments WHERE id=?', [req.params.id]);
    await logAction('filament', req.params.id, 'weigh', 'Pesée : ' + Math.round(weight_remaining) + 'g — ' + (f?.name||''));
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
