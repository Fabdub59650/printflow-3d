...
async function openWeighingHistory(filamentId) {
  ...
  try {
    ...
    const totalConsumed = history.reduce((sum, w, i) => {
    ...
    openModal(`
      <div style="display:flex;gap:12px;margin-bottom:16px">
        <div class="metric-card" style="flex:1">
          <div class="metric-label">Pesées enregistrées</div>
          <div class="metric-value">${history.length}</div>
        </div>
        <div class="metric-card" style="flex:1">
          <div class="metric-label">Dernière pesée</div>
      ...
      <table>
        ...
        <tbody>
          ${history.map(w => {
            ...
            const diffColor = diff > 5 ? 'var(--success)' : diff < -5 ? 'var(--danger)' : 'var(--text3)';
            return `<tr>
    ...
    `, 'Historique pesées — ' + filamentName);
...
function exportFilamentsPDF() {
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text('PrintFlow v1.6.0 — Page ' + i + ' / ' + pageCount, 14, 205);
  }
